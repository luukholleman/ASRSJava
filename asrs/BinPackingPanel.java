...
public class BinPackingPanel extends JPanel implements Runnable, BinPacking {
	...
	public BinPackingPanel() {
		...
		//De hoogte van de lijntjes die worden getekend op de lopende band staan in een array.
	...
	public void paintComponent(Graphics g) {
		...
		g.drawRect(CONVEYOR_X + STRING_SPACE, CONVEYOR_Y, CONVEYOR_WIDTH, CONVEYOR_LENGTH);
		...
		g.drawString(overflow + "/INFINITY", OVERFLOW_WIDTH/2-5, OVERFLOW_LENGTH/2-5);
		
		// Tekent de lijntjes van de lopende band.
		for (int line : lines) {
			g.drawLine(CONVEYOR_X + STRING_SPACE, line, CONVEYOR_X + STRING_SPACE + CONVEYOR_WIDTH, line);
		}
	...
	private void drawProduct(Graphics g) {
		//Het product wordt in het midden van de lopende band getekend door zijn linker boven hoek de helft van zijn grootte naar links van het midden van de band te bewegen.
		g.drawRect(CONVEYOR_X + STRING_SPACE + CONVEYOR_WIDTH / 2 - PRODUCT_SIZE / 2, productY, PRODUCT_SIZE, PRODUCT_SIZE);
		g.setColor(Color.cyan);
		g.fillRect(CONVEYOR_X + STRING_SPACE + CONVEYOR_WIDTH / 2 - PRODUCT_SIZE / 2 + 1, productY+1, PRODUCT_SIZE-1, PRODUCT_SIZE-2);
		g.setColor(Color.BLACK);
		//Als er geen product op de lijn staat kan hij de grootte ervan niet ophalen. Deze wordt in het midden van het product getekend.
		if(!productLine.isEmpty())
				g.drawString(Integer.toString(productLine.get(0).getSize()), CONVEYOR_X + STRING_SPACE + CONVEYOR_WIDTH / 2 - PRODUCT_SIZE_STRING_ADJUSTMENT, productY + PRODUCT_SIZE / 2 + PRODUCT_SIZE_STRING_ADJUSTMENT);
	}
	...
	private void drawBins(Graphics g) {
		for (Bin bin : bins) {
			//Voor deze formule kreeg ik hulp van Tim. Om de bin wordt er horizontaal de afstand bij opgeteld en daarna afgetrokken. Om elke 2 bins worden ze de verticale afstand omlaag gezet.
			int binx = bins.indexOf(bin) % 2 * BIN_HORIZONTAL_DISTANCE + STRING_SPACE;
			...
			//De diepte geeft aan hoe vol hij al is, zodat er daar bovenop kan worden getekent
			int depth = 1;
			...
			for(Product product : bin.getProducts()){
				...
				float productCalc = (float) (BIN_SIZE-2)/bin.getSize()*product.getSize();
				int productDiv = (int) productCalc;
				//Teken eerst de binnenkant van het product
				g.setColor(Color.cyan);
				g.fillRect(binx, biny+BIN_SIZE-depth-productDiv, BIN_SIZE, productDiv);
				//Teken daarna een zwarte rand er omheen
				g.setColor(Color.BLACK);
				g.drawRect(binx, biny+BIN_SIZE-depth-productDiv, BIN_SIZE, productDiv);
				//Opslaan hoe vol de bin is, zodat het volgende product er op glegd kan worden
				depth = depth + productDiv +1;
				//Om elk product wordt de grootte van het product aan de andere kant van de bin getekent.
				if(side)
					g.drawString(Integer.toString(product.getSize()), binx+BIN_SIZE+STRING_INDENT_X, biny+BIN_SIZE+STRING_INDENT_Y-depth);
				else
					g.drawString(Integer.toString(product.getSize()), binx-STRING_SPACE, biny+BIN_SIZE+STRING_INDENT_Y-depth);
				side = !side;
			}
		}
	}
	...
	public void run() {
		while (runner != null) {
			...
			if (!productLine.isEmpty())
				productY--;
			//Als het product bovenaan is aangekomen, doe het product in een bin.
			if (productY <= CONVEYOR_Y) {
				...
				executionManager.getMain().productStatusUpdated(productLine.get(0));
	...
	public void packProduct(Byte binByte, Product product) {
		...
		//Als de meegegeven byte gelijk is aan de .size() van de array, geef dan 'null' mee om aan te geven dat hij nergens in past
	 ...
	 */
	private void frame() {
		//Omdat ik niet steeds deze try/catch in de code wil hebben staan, heb ik hier een private method gemaakt.
		try {
			Thread.sleep(20);
