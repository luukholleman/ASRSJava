...
public class OrderPickingPanel extends JPanel implements Runnable, Warehouse {
	...
	public OrderPickingPanel(BinPackingPanel bpPanel) {
		...
		robotLeft.pixels = new Location(BINPACKER_SIZE + ROBOT_INDENT
				+ (robotLeft.location.x * CELL_SIZE), ROBOT_INDENT
				+ ((WAREHOUSE_MAX_Y - robotLeft.location.y) * CELL_SIZE));
		
		robotRight = new WarehouseRobot(getStartLocation(1), 1);
		robotRight.pixels = new Location(BINPACKER_SIZE + ROBOT_INDENT
				+ (robotRight.location.x * CELL_SIZE), ROBOT_INDENT
				+ ((WAREHOUSE_MAX_Y - robotRight.location.y) * CELL_SIZE));
	...
	private void drawRobots(Graphics g) {
		...
		for (WarehouseRobot robot : robots) {
			g.fillRect(robot.location.x + 7 - (robot.load * 2), robot.location.y + 7
	...
	public void run() {
		...
		while (runner == thisThread) {
			...
			if (robots[0].load <= LOAD_MAX || robots[1].load <= LOAD_MAX) {
				...
				for (WarehouseRobot robot : robots) {
					if (robot.location.x == BINPACKER_X
							&& robot.location.y == BINPACKER_Y)
	...
	private void move() {
		...
		int stepx0 = robots[0].destination.x - robots[0].location.x;
		int stepy0 = robots[0].destination.y - robots[0].location.y;

		int stepx1 = robots[1].destination.x - robots[1].location.x;
		int stepy1 = robots[1].destination.y - robots[1].location.y;
