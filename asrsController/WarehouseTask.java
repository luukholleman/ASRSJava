...
public class WarehouseTask {
	...
	protected boolean problemOkay(int problemNumber)
	{
		...
		for(int i=0; i<noi; i++) check[i] = false;
		for(int rob=0; rob<2; rob++)
		{
			for(int i=0; i<maxNumberOfItems; i++)
			{
				...
				if(check[item])
				{
					System.out.print("Item "+ item + " occurs more than once in problem " + problemNumber + "\n");
					result = false;
				}
				else
				{
		...
		for(int i=0; i<noi; i++)
		{
			if(!check[i])
			{
				System.out.print("Item " + i + " missing in problem number " + problemNumber + "\n");
				result = false;
			}
		}
		return result;
	...
	protected double calculateSinglePath(int problemNumber, int robot)
	{
		...
		for(int i=0; i<numberOfItems[problemNumber] && solution[problemNumber][robot][i] != -1; i++)
		{
			int item = solution[problemNumber][robot][i];
                        x2 = problem[problemNumber][item][0];
			y2 = problem[problemNumber][item][1];
			length = length + Math.sqrt(Math.pow(x2-x1,2)+ Math.pow(y2-y1,2));    
                        x1 = x2;
			y1 = y2;
		}
		if(robot ==0)
	...
	protected void calculateResult()
	{
		if(errorCount > 0)
		{
			System.out.println("Number of errors = " + errorCount + ", no calculation of result.");
		}
		boolean resultOkay = true;
		for(int problemNumber = 0; problemNumber < numberOfProblems; problemNumber++)
		{
			if(!problemOkay(problemNumber)) resultOkay = false;
		}
		if(resultOkay)
		{
			doCalculation();
	...
	public int getNumberOfItems(int probl)
	{
		if(processRunning)
		{
			...
			{
				errorCount++;
				System.out.println("Array index out of bounds in getNumberOfItems");
				return -1;
			}
		}
		else
		...
		if(processRunning)
		{
			...
			else
			{
				...
				else
				{
					if(robot != 0 && robot != 1)
					{
						System.out.println("robot number out of bounds in setOrder");
						errorCount++;
					}
					else
					{
						...
						{
							System.out.println("sequence number out of bounds in setOrder");
							errorCount++;
						}	
						else
						{
