...
public class BinPackingTask {
    ...
    private int[][] problems;       // problem_number item_number (defines the problems to be solved)
    private int[] numberOfItems;    // problem_number (nr of items differs between problems)
    private int[][] solutions;      // problem_number item_number (stores in which bin the item is stored)
    ...
    private boolean debug; // moet er op console debug-informatie getoond worden?

    /**
     * Constructor for Task2, no debugging 
     * 
     * @param  seed  a seed to feed the randomizer
     * @see          Task2
     */
    ...
    /**
     * Constructor for Task2. This constructor fills the list of problems.
     * A problem is a list of itemsizes. These are the product sizes that have to be used by the to be designed algorithm
     * 
     * @param  seed  a seed to feed the randomizer
     * @param  debug a boolean to decide whether or not to show debug
     *               info on the console
     */
    ...
    /**
     * fillProblems is a helper function that creates the random items.
     * All items in a problem may never sum up above MAXTOTALSIZE.
     * 
     * @param  seed  a seed to feed the randomizer
     */
     ...
     * 
     * @param  pr  the problem index
     * @param  item  the item index
     * @param  size  the specific size of the item
     * @see          Task2
     */
    private void fillProblem(int pr, int item, int size) {
        problems[pr][item] = size;
        if (debug) {
            System.out.println("Fill problem " + pr + " with item " + item + " (size " + size + ")");
        }
    }

    /**
     * calculateNrOfBins is used in the doCalculation method to calculate
     * the nr of used bins per problem
     *
     * @param problemNumber the problem index
    ...
    private int calculateNrOfBins(int problemNumber) {
        ...
        for (int i = 0; i < numberOfItems[problemNumber] && solutions[problemNumber][i] != -1; i++) {
    ...
    /**
     * calculateSpaceLeft is used by the doCalculation method to get the sum of all space left in used bins.
     * 
    ...
    private int calculateSpaceLeft(int problemNumber) {
        ...
        for (int b = 0; b < MAXNUMBEROFBINS; b++) {
            if (filling[b] > BINSIZE) {
                ...
                System.out.println("Error: Bin " + b + " is overpacked in problem " + problemNumber + "\n");
            }
            if (filling[b] > 0) {
    ...
    /**
     * doCalculation shows all the results:<p>
     * number of bins needed over all problems,
     * empty space in all used bins over all problems,
    ...
    /**
     * startProcess starts the time and process. It has to be called before your algorithm.
     * 
     */
    ...
    /**
     * finishProcess stops the time and starts calculation of performance. It has to be called after your algorithm
     * 
     */
    ...
    public int getNumberOfItems(int probl) {
        if (processRunning) {
            if (probl >= 0 && probl < NUMBEROFPROBLEMS) {
                ...
                if(debug)System.out.println("Array index out of bounds in getNumberOfItems");
                return -1;
            }
        } else {
            if(debug)System.out.println("Process not running");
            return -1;
        }
    }
     ...
     * 
     * @param problem_number   the problem to be tackled
     * @param item             the item to put in a bin
     * @param bin              the binnumber where to put it
     */
    public void setBin(int problem_number, int item, int bin) {
        if (processRunning) {
            if (problem_number < 0 || problem_number >= NUMBEROFPROBLEMS) {
                if(debug)System.out.println("Problem number out of bounds in setBin");
            } else {
                if (item < 0 || item >= numberOfItems[problem_number]) {
                    if(debug)System.out.println("Item number out of bounds in setBin");
                } else {
                    if (bin < 0 || bin > MAXNUMBEROFBINS) {
                        if(debug)System.out.println("Bin number out of bounds in setBin");
                    } else {
                        if (debug) {
                            System.out.println("Problem " + problem_number + ": Put item " + item + " with size " + problems[problem_number][item] + " in bin " + bin);
                        }
    ...
    /**
     *  calculateSpaceLeftInBin can be used to determine how many space there still is in a bin
     *  @param problemNumber the number of the problem
